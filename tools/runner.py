...
import faiss
import gc
from tools.loss import compute_all_loss, DomainLoss


class Trainer:
    def __init__(self, model_s, model_t, train_loader, whole_train_loader, whole_val_set, whole_val_loader,
                 device, num_epochs, resume_path_s, resume_path_t, log, log_dir, ckpt_dir, cache_dir,
        ...
        self.resume_path = resume_path_s
        self.resume_path_t = resume_path_t
        self.log = log
    ...
    def train(self):
        ...
        # self.model_t.load_state_dict(torch.load(self.resume_path_t))
        if self.resume_path is not None:
            print("Resuming from ", self.resume_path)
            checkpoint = torch.load(self.resume_path)
            starting_epoch = checkpoint['epoch']
            self.model_s.load_state_dict(checkpoint['net'])
            del checkpoint
        else:
            print("Training from scratch ...")
            starting_epoch = 0

        self.optimizer = Adam(filter(lambda p: p.requires_grad, self.model_s.parameters()),
                              lr=self.lr)
        ...
        for epoch in range(starting_epoch + 1, self.num_epochs):
            ...
            for i, (input_dict, nNeg) in enumerate(self.train_loader):
                ...
                batch_size = len(camera_feature)
                domain_label_s = torch.zeros(batch_size).to(self.device)
                domain_label_t = torch.ones(batch_size).to(self.device)

                nNeg = nNeg[0]

                len_loader = len(self.train_loader)
                p = float(i + epoch * len_loader) / self.num_epochs / len_loader
                alpha = 2. / (1. + np.exp(-10 * p)) - 1

                self.optimizer.zero_grad()

                global_des_s, domain_s = self.model_s(camera_feature)
                global_des_t, domain_t = self.model_t(lidar_feature)
                del camera_feature, lidar_feature

                domain_s = domain_s.view(-1)  # 展平预测值
                domain_t = domain_t.view(-1)  # 展平预测值

                neg_des, pos_des, query_des = torch.split(
                    global_des_s, [nNeg, 1, 1], dim=0)

                query_des = query_des.expand(nNeg, -1)
                pos_des = pos_des.expand(nNeg, -1)

                # task loss
                task_loss = nn.functional.triplet_margin_loss(query_des, pos_des, neg_des, margin=self.margin)
                # distil loss
                output_dict_stu = {'embedding': global_des_s}
                output_dict_tea = {'embedding': global_des_t}
                distil_loss = compute_all_loss(output_dict_stu, output_dict_tea)
                # dis loss
                domain_loss = self.domain_loss(domain_s, domain_label_s)
                domain_loss += self.domain_loss(domain_t, domain_label_t)

                loss = task_loss + self.lambda_m * distil_loss + self.lambda_d * domain_loss
        ...
        with torch.no_grad():
            ...
            with h5py.File(cache, mode='w') as h5:
                ...
                for i, input_dict in enumerate(tqdm(self.whole_val_loader)):
                    ...
                    descriptor, _ = self.model_s(camera_feature)
    ...
    def get_feature(self, ckpt_path, feature_path):
        ...
        with torch.no_grad():
            ...
            for i, input_dict in enumerate(tqdm(self.whole_test_loader)):
                ...
                descriptor = self.model_s(camera_feature)
