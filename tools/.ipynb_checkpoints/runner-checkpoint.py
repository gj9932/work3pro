    ...
    def train(self):
        ...
        for epoch in range(starting_epoch + 1, self.num_epochs):
            ...
            for i, (input_dict, nNeg) in enumerate(self.train_loader):
                ...
                domain_label_s = torch.zeros(batch_size).long().to(self.device)
                domain_label_t = torch.ones(batch_size).long().to(self.device)
                ...
                # distil loss
                distil_loss = compute_all_loss(global_des_s, global_des_t)
                # dis loss
