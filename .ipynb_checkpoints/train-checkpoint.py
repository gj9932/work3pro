...
from modules.S_net import SPR
from modules.T_net import TPR
...
def main():
    ...
    resume_path_s = cfg['runner']['resume_path']
    resume_path_t = cfg['runner']['resume_path']
    log = cfg['runner']['log']
    ...
    model_s = SPR.create(weights=ResNet18_Weights.DEFAULT)
    model_t = TPR.create(weights=ResNet18_Weights.DEFAULT)

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    trainer = Trainer(model_s, model_t, train_loader, whole_train_loader, whole_val_set, whole_val_loader, device,
                      num_epochs, resume_path_s, resume_path_t,log, log_dir, ckpt_dir, cache_dir,
