...
我们提出 **GeoDistill-VLM**，在冻结 Qwen2.5-VL 原生视觉路径上将训练期 LiDAR 几何蒸馏到视觉 token。核心方法 **Native Token–LiDAR Fused Geometric Transport（NTL-FGT）** 将 Qwen 原生视觉 token 与 LiDAR 几何元素分别建模为带属性的离散测度，并通过 feature-level transport cost 与 FGW-inspired fixed-structure cost 学习 soft token-LiDAR coupling。该 coupling 的逐 token concentration-aware representative 提供 metric depth、三维坐标和可靠度 teacher，避免将单个硬投影、局部分位数或跨表面的简单均值直接视为唯一监督。

在该 teacher 上，**Risk-Reliability Adaptive Companding（R²AC）** 将车辆动力学风险与视觉可预测的几何可靠度转化为 token 级连续压扩强度。随后，semantic-preserving geometry injection 将预测位置与关系分别编码为 3D PE 和 relation residual，并通过 zero-init gate 与 delayed semantic constraint 注入原生视觉 token。LiDAR、ego pose、标定和可选 3D 标注仅在训练期构造监督；部署时只使用多视角图像、标定和标准 ego state，不使用 LiDAR、最优传输求解器或外部深度估计器。我们在深度精度—效率权衡、驾驶空间问答、跨视角几何、nuScenes 开环规划、Bench2Drive 闭环驾驶和相机退化鲁棒性上评估该设计。
...
训练时，我们先在冻结的 `image_embeds` 与 LiDAR 几何测度之间构造 NTL-FGT teacher，再由轻量适配器学习该 teacher。主文围绕三个核心模块组织：

1. **NTL-FGT teacher construction**：通过 feature-structure soft coupling 与 concentration-aware representative 构造 token-level metric geometry teacher；
2. **R²AC depth representation**：根据驾驶风险与可预测可靠度分配连续深度压扩强度；
3. **Semantic-preserving geometry injection**：将预测三维位置与跨 token 关系分别编码为 3D PE 和 relation residual，并以零初始化门控和语义保持约束注入原生 token。

typed validity mask、F0/F1/F2 优化伪代码和低层超参数消融属于 NTL-FGT 的实现细节，统一放入附录。
...
其中 `H_img` 是冻结 Qwen merger 输出，`C_hat` 是轻量适配器预测的三维坐标，`Phi` 是 universal 3D positional encoder，`DeltaH_rel={Delta h_p^rel}` 是关系蒸馏得到的几何残差。两个门控系数均从零初始化，并在 geometry warm-up 后逐步启用 risk-aware semantic relation constraint：高风险 token 允许更多几何残差，低风险 token 更强保持原生语义结构。
...
1. 提出 **Native Token–LiDAR Fused Geometric Transport**，将训练期 LiDAR 几何测度与冻结 Qwen 原生视觉 token 建模为 FGW-inspired feature-structure transport 问题，通过 soft token-LiDAR coupling 和 concentration-aware transport representative 构造 token-level 几何 teacher。
2. 提出 R²AC 连续深度表示，将车辆动力学风险、局部几何一致性与 transport concentration 统一为 token 级压扩变量；该表示严格单调、解析可逆，并具有显式可控的近远场监督灵敏度比。
3. 提出 **semantic-preserving geometry injection**，以 explicit 3D PE 表达 token 位置、以稀疏 relation residual 表达跨视角与遮挡结构，并通过 zero-init gate、delayed local relation preservation 和全局语义监控限制非必要语义漂移。
...
最优传输通过 coupling 对齐两个离散测度，熵正则可使用 Sinkhorn 迭代高效近似 [36]。Gromov-Wasserstein 距离进一步比较两个测度内部的成对关系，而不要求两侧元素处于同一特征空间 [37]；Fused Gromov-Wasserstein 同时保留元素级 feature cost 和结构级关系 cost [38]。本文受该思想启发，但不将目标表述为严格的标准 FGW distance：token 侧同时包含固定图像/射线结构和 learned typed relation predictor，LiDAR 侧则混合连续距离、可见性与离散对象/表面属性。我们因此构造面向 token-LiDAR teacher construction 的 FGW-inspired structured transport objective，以固定结构主导 coupling，以停止梯度的 learned typed relation 提供辅助结构监督。
...
- shared relation edge encoder 与 typed relation heads；
...
LPGA 是承载 R²AC 与 geometry injection 的轻量实现，而非独立贡献。其内部仅分为三组：

1. **Token geometry encoder**：融合 Qwen image token、camera ID 和归一化图像位置；
2. **R²AC geometry heads**：联合预测连续压扩深度、风险/可靠度分解和 sub-token ray offset；
3. **Geometry injection heads**：预测稀疏 token 关系，并生成 gated 3D PE 与 relation residual。
...
该策略保留前景表面，同时比单个最小值对离群点更稳健。令 `m_p^Q=1[D_p != empty]` 表示 token 具有有效 hard depth label；`min depth` 作为与 SpaceDrive 对齐的消融。主方法不再把 `d_p^Q` 视为唯一 teacher，而是在第 3.5 节通过 soft coupling 构造 concentration-aware transport teacher；`d_p^Q` 仅作为 hard-assignment 基线和可选 fallback。
...
其中 `omega_j` 由时间距离和投影置信度确定，`delta_p^Q` 截断到 `[-1,1]^2`。该 hard offset 在消融中与 concentration-aware transport ray teacher 比较。
...
其中 `d_i` 是 `c_i` 的相机深度。主方法固定 `lambda_d=0`，使 coupling 仅由标定、投影、射线和可见性确定，避免当前 student depth 反过来强化自身错误。`d_p^prior=StopGrad(d_hat_p)` 只作为 late-stage optional refinement 或消融，并始终停止梯度。投影范围、正深度和稀疏 z-buffer visibility 作为 hard candidate mask，`lambda_cam` 只对剩余候选施加软惩罚。

#### 固定结构与可学习 typed structure

元素级投影仍可能在重复纹理、跨视角重叠和稀疏远场产生歧义。为避免 learned structure predictor 与 coupling 相互追逐，我们将 token-side structure 拆分为固定结构与可学习结构。固定结构不含可训练参数：

```text
C_pq^(V,fix) = [
  ||e_uv(p) - e_uv(q)||_2,
  1 - <rho_p^ego, rho_q^ego>,
  camera_separation(p,q),
  ||StopGrad(LN(h_p)) - StopGrad(LN(h_q))||_2 / sqrt(D)
]
```

其中 `rho_p^ego` 是 token ray 转到 ego 坐标后的单位方向，`camera_separation in [0,1]` 将同相机/重叠相机映射为较小值，将非重叠相机映射为较大值。第四项 frozen-token feature distance 只作为弱 **semantic compatibility cue**，不作为 metric geometry cue；固定几何锚点来自图像位置、ego-ray angle 和 camera overlap。该项只读取冻结 Qwen token，并停止梯度。对应的 LiDAR 固定结构为：

```text
C_ij^(L,fix) = [
  Normalize(||c_i - c_j||_2),
  1 - <rho_i^ego, rho_j^ego>,
  1 - co_visibility_ij,
  1 - 1[same voxel or surface]
]
```

其中 `rho_i^ego=normalize(c_i)`；四个分量均归一化到 `[0,1]`，并通过预先确定的分量权重比较：

```text
D_fix(C_pq^(V,fix), C_ij^(L,fix)) =
  sum_k m_(p,q,i,j,k) lambda_k^fix
    |C_pq^(V,fix)[k] - C_ij^(L,fix)[k]|
  / (sum_k m_(p,q,i,j,k) lambda_k^fix + epsilon)
```

`m_(p,q,i,j,k)` 是 typed validity mask，仅比较当前边类型下可解释的结构分量；具体启用规则见附录 A.1。

可学习结构直接复用第 3.10 节推理期 relation adapter 的共享 edge encoder `A_edge` 与 typed relation heads。记这些部署时仍保留的 typed heads 合成为 `A_struct^V`：

```text
C_pq^(V,pred) = A_struct^V(r_pq)

C_ij^(L,type) = [
  Normalize(||c_i - c_j||_2),
  sign(d_j - d_i),
  1[same object or same voxel/surface],
  topology_ij,
  occlusion_ij
]

D_typed(C_pq^(V,pred), C_ij^(L,type)) =
  sum_(k in K_cont)
    lambda_k Huber(
      C_pq^(V,pred)[k],
      C_ij^(L,type)[k]
...
`C^(V,fix)` 是 coupling 的主要结构依据；`C^(V,pred)` 主要接受 typed relation supervision。只有在 predictor 已由固定 coupling 校准后，才允许其停止梯度版本以受限权重 `kappa_pred` 辅助 coupling，避免自由 predictor 完全决定匹配。

#### FGW-inspired fused geometric transport objective

定义用于更新 coupling 的结构差异：
...
L_struct^pred =
  sum_(p,q,i,j)
    StopGrad(pi_(p,i) pi_(q,j))
    D_typed(C_pq^(V,pred), C_ij^(L,type))

L_FGT =
  L_cpl
  + lambda_struct^pred L_struct^pred
```

`L_cpl` 的第一项对齐 token-LiDAR 的局部投影、射线与可见性；第二项以固定结构为主对齐 token-token 与 LiDAR-LiDAR 的成对几何；第三项为 entropic regularization。`L_struct^pred` 在固定 coupling 下训练 typed relation predictor。`gamma=0` 退化为只使用元素级 cost 的熵正则 OT。

该目标受 FGW 的 feature-structure joint alignment 启发，但不是严格标准 FGW distance：两侧结构包含不同类型的固定几何量，且另有 learned typed predictor 与监督项。因此本文将其称为 **FGW-inspired fused geometric transport objective**，用途限定为 token-LiDAR teacher construction，不主张满足标准 FGW distance 的全部定义。

#### 稳定的 teacher 优化

为切断 predictor、coupling 与 metric heads 的循环依赖，我们使用三步停止梯度日程：先由固定 cost 初始化 coupling，再固定 coupling 训练 typed relation predictor，最后仅以停止梯度的 predictor 小权重辅助 coupling 更新并构造 teacher。固定结构始终主导 coupling，metric heads 不能通过改变 `pi` 降低自身损失。F0/F1/F2 伪代码、停止梯度位置与稀疏实现见附录 A.2；该日程是 teacher construction 的优化实现，不作为独立方法贡献。

#### Concentration-aware transport teacher

对 token `p`，先定义 coupling 的行归一化分布和 barycentric projection：
...
bar c_p^L =
  sum_(i in I_p) StopGrad(tilde pi_(p,i)) c_i
```

简单 barycenter 在单峰 coupling 下稳定，但当 coupling 同时覆盖前景与背景时，可能落在两个真实表面之间。令 `I_p` 为有效候选，定义 concentration 与 transported mass：
...
当 `|I_p|=1` 时定义 `q_p^conc=1`。高 concentration token 使用 barycenter；低 concentration token 在 top-k support 上选取 transport medoid：

```text
J_p^k = TopK(I_p, tilde pi_(p,:), k)

i_p^med =
  argmin_(i in J_p^k)
  sum_(j in J_p^k)
    tilde pi_(p,j) ||c_i - c_j||_2

c_p^med = c_(i_p^med)

c_p^T =
  bar c_p^L,  if q_p^conc >= tau_conc
  c_p^med,    otherwise
```

medoid 始终选择实际 LiDAR anchor，避免在多模态 support 之间生成物理上不存在的三维点。Top-k normalized barycenter 和沿相机射线选择最近可靠 mode 的 front-biased representative 作为消融。主方法进一步采用保守的 concentration gate：
...
`s_p` 使单点或低支持度 hard statistics 不会主导融合；当没有 hard region statistics 时，可靠度退化为 `q_p^OT=q_p^conc q_p^mass`。由于 `q_p^conc` 位于融合式外部，即使 `q_p^geom` 较高，多模态 coupling 也不会获得高 teacher weight。

该设计是需要实验验证的保守假设，而非预设最优形式。medoid 已避免多模态 support 产生物理无效均值，而低 concentration 也可能来自物体边界、小目标或遮挡区域；再次强降权可能使这些区域监督不足。令
...
我们比较三种外部门控：`q_p^conc g_p`（主方法）、`q_p^mass g_p` 和 `sqrt(q_p^conc q_p^mass) g_p`，并按边界、小目标、遮挡与普通区域分别报告 depth/ray/coordinate error。

并定义主 teacher：

```text
d_p^T =
  [T_ego->cam(p) c_p^T]_z
...
其中 `[.]_z` 表示相机坐标系中的正向 `z` 分量。将 `c_p^T` 投影回 token 相机可得到 transport ray teacher：
...
delta_p^T = [
  2 (u_p^T - u_p) / width(R_p),
  2 (v_p^T - v_p) / height(R_p)
]
```

后续 R²AC、ray 和 coordinate supervision 默认使用 `(d_p^T,c_p^T,delta_p^T,q_p^teacher)`。Quantile teacher `(d_p^Q,delta_p^Q)` 仅用于 hard projection 对照或 coupling 失败时的 fallback。`StopGrad(pi)` 使 depth/coordinate heads 不能通过改变 coupling 降低自身 loss；low-concentration teacher 则通过 `q_p^teacher` 在所有几何损失中一致降权。
...
令 `x_p^ego` 和 `y_p^ego` 分别为 transport representative `c_p^T` 在 ego 坐标系中的前向与横向距离，定义软风险：
...
该消融用于检验组合目标的联合梯度是否损害两个因子的独立校准。`q_p^mass` 控制 partial transport teacher 的监督覆盖度，风险分支使用所有具有有效 transport representative 的 token。`StopGrad` 阻止 allocation head 通过改变逆变换来吸收 metric depth 误差；allocation 只能通过 `L_alloc` 学习可解释因子。由于不同 token 使用不同压扩强度，不能直接比较其 `zeta_hat`。因此排序损失作用于解码后的 metric depth：
...
其监督目标为第 3.5 节由 concentration-aware transport representative 投影得到的 `delta_p^T`：
...
训练时，对存在有效 transport representative 的 token 直接计算坐标误差：
...
因此 student 同时对齐 transport depth、representative ray 与三维位置。固定 token center、hard quantile 占用质心、high-concentration barycenter、low-concentration medoid 和预测 ray offset 将在消融中比较。
...
可见性通过每个相机的稀疏 z-buffer 检查：候选点投影深度必须与该像素邻域的最近可靠深度一致。外观相似但 voxel、object ID 或可见性不一致的候选构成 hard negative。边可靠度定义为两个节点 `q_p^teacher`、`q_q^teacher`、coupling overlap、体素重叠率与深度一致性的乘积，并用于过滤关系损失。NTL-FGT 的 fixed structure term 约束 teacher coupling，typed predictor 与 `L_rel` 则训练共享的推理期 relation representation，二者互补而非替代。训练时使用 LiDAR/3D box 构造标签；推理时不构图标签，只在图像局部边和预测三维位置的 kNN 边上运行 relation adapter。
...
`A_edge^same/A_edge^cross` 与第 3.5 节 F1 使用的 edge encoder 是同一组参数；`A_struct^V` 也由下述 relation prediction heads 直接实现。NTL-FGT 不复制另一套 relation predictor，训练期独有的仅是 `D_fix/D_typed` cost assembly、coupling construction 与 OT solver。relation heads、confidence aggregation 和 `W_up` 在推理时保留。
...
第 3.5 节的 `C_pq^(V,pred)` 由这些现有输出组成：连续距离分量使用 `||delta_c_hat_pq||_2`，离散分量复用 order、cross-view、topology 和 occlusion logits，不增加另一套 predictor。
...
令 `H_geo={h_tilde_p}`。原有 `L_keep` 只约束全局 pooled representation，不能阻止局部 token relation 被任意重排；但直接对全部 token 施加强 CKA 也可能压制安全关键区域所需的几何 residual。我们因此使用 risk-aware local relation preservation，并将 CKA [39] 保留为全局监控指标。定义：

```text
s_pq^geo =
  cosine(LN(h_tilde_p), LN(h_tilde_q))

s_pq^img =
  StopGrad(cosine(LN(h_p), LN(h_q)))
...
a_p^sem =
  a_p^*,               if geometry teacher is valid
  StopGrad(a_hat_p),    otherwise

w_p^sem =
  1 - StopGrad(a_p^sem)

L_sem =
  sum_p w_p^sem ell_p^sem
  / (sum_p w_p^sem + epsilon)
```

`N_sem(p)` 由同相机局部 token 和少量跨视角语义近邻组成。geometry minibatch 使用 oracle `a_p^*`，无 LiDAR 的语言 minibatch 使用停止梯度的预测 `a_hat_p`。高风险 token 允许更大的几何变化，低风险 token 则更强保持原生语义关系。为避免 geometry adapter 尚未成形时受到过早约束，阶段 B 使用：

```text
lambda_sem(t) =
  0,  t <= T_geo_warm
  lambda_sem^max
    Ramp((t - T_geo_warm) / T_sem_ramp),
  otherwise
```

其中 `Ramp` 为线性或 cosine ramp。zero-init gate、risk-aware weighting 与 delayed schedule 共同避免 `L_sem` 过强压制几何注入。全局 `CKA(H_geo,H_img)` 仍用于报告 semantic retention，并作为无 risk weighting 的对照损失。
...
阶段 A 在每个 geometry batch 内按附录 A.2 的停止梯度日程构造 teacher，并用冻结的 concentration-aware teacher 热启动 allocation、ray 和 relation heads：
...
随后加入由 concentration-aware transport teacher 固定构造的 oracle 压扩目标 `zeta_p^*=F(d_p^T;a_p^*)`、metric coordinate 和 ranking 监督。完整阶段 A 目标为：
...
`A0` 与完整阶段 A 使用相同的 `lambda_alloc`、`lambda_ray`、`lambda_rel` 和 `lambda_fgt`。主方法在全部阶段固定 `lambda_d=0`；stopped-gradient predicted depth prior 只在单独消融中 late-stage 启用。最终 `pi` 在构造 teacher 前停止梯度，因此 metric heads 不能反向改变 coupling。

阶段 A 将 `alpha_pe` 和 `alpha_rel` 固定为 0，不向 LLM 注入几何。relation supervision 此时训练 shared edge encoder 与 typed relation heads，`W_up` 不参与阶段 A。
...
- shared edge encoder 与 typed relation heads；
- LPGA；
...
阶段 B 中 `lambda_fgt` 小于阶段 A，仅在包含同步 LiDAR 的 geometry minibatch 上启用，用于防止 instruction tuning 后 coupling、depth teacher 和 relation structure 漂移；无 LiDAR 的语言 minibatch 令该项为 0。`lambda_sem(t)` 在 geometry warm-up 后从 0 逐步升高，`L_sem` 与 `L_keep` 分别限制 risk-aware token relation drift 和全局 pooled drift。
...
| LiDAR teacher / OT solver / coupling | 构造 teacher | 仅 geometry minibatch 低权重使用 | 移除 |
| Shared relation edge encoder / typed relation heads | 训练 | 训练 | 使用 |
| Training-only cost assembly `D_fix/D_typed` | 构造 objective | 仅 geometry minibatch 使用 | 移除 |
...
| Depth-prior weight `lambda_d` | 0（主方法） |
| Partial transport budget `m_tr` | TBD |
| Teacher reliability fusion `eta_q` | TBD |
| Partial mass threshold `tau_mass` | TBD |
| Concentration threshold `tau_conc` | TBD |
| Low-concentration medoid top-k | TBD |
| Predictor coupling cap `kappa_pred` | TBD |
| F1/F2 alternating rounds | TBD |
| Semantic warm-up `T_geo_warm` | TBD |
| Semantic ramp `T_sem_ramp` | TBD |
...
停车距离采用常减速度运动学模型。上述参数是默认训练配置而非道路法规常数，并对 `gamma`、`epsilon_ot`、`kappa_pred`、`tau_conc`、F1/F2 轮数、候选 anchor 数、`eta_q`、`t_react`、`a_brake`、`w_corridor`、`D_max` 和 `exp(beta)` 做敏感性分析；同时报告训练集 `d_safe`、transported mass、coupling entropy 与 teacher construction overhead，避免只给单点超参数。
...
为使 NTL-FGT 的主任务收益可直接比较，表 1、表 4 和表 5 均显式拆分 hard quantile teacher、Entropic OT teacher 与 Full NTL-FGT 三行，并保持其余训练预算一致。
...
| GeoDistill-VLM w/ hard quantile teacher | TBD | TBD | TBD | TBD | TBD | TBD | TBD | TBD |
| GeoDistill-VLM w/ Entropic OT teacher | TBD | TBD | TBD | TBD | TBD | TBD | TBD | TBD |
| GeoDistill-VLM w/ Full NTL-FGT | TBD | TBD | TBD | TBD | TBD | TBD | TBD | TBD |
...
| GeoDistill-VLM w/ hard quantile teacher | TBD | TBD | TBD | TBD | TBD | TBD |
| GeoDistill-VLM w/ Entropic OT teacher | TBD | TBD | TBD | TBD | TBD | TBD |
| GeoDistill-VLM w/ Full NTL-FGT | TBD | TBD | TBD | TBD | TBD | TBD |
...
| GeoDistill-VLM w/ hard quantile teacher | TBD | TBD | TBD | TBD | TBD |
| GeoDistill-VLM w/ Entropic OT teacher | TBD | TBD | TBD | TBD | TBD |
| GeoDistill-VLM w/ Full NTL-FGT | TBD | TBD | TBD | TBD | TBD |
...
**表 7：训练期 teacher construction overhead。**

| Teacher construction | Time / batch | Extra memory | Candidate anchors / token | Sinkhorn iterations | Sparse edges / token |
|---|---:|---:|---:|---:|---:|
| Hard quantile teacher | TBD | TBD | TBD | N/A | N/A |
| Entropic OT teacher, `gamma=0` | TBD | TBD | TBD | TBD | N/A |
| Full NTL-FGT | TBD | TBD | TBD | TBD | TBD |

该表只统计训练期 teacher construction，不计入表 6 的 camera-only 推理 latency 与显存。Full NTL-FGT 需报告完整三步停止梯度日程的实际开销，分项实现见附录 A.2。

**表 8：NTL-FGT teacher 与语义保持消融。**

| Teacher / Transport | Feature | Fixed struct. | Typed pred. | Entropic | `L_sem` | AbsRel ↓ | Cross-view ↑ | CKA ↑ | `rho(q^OT,error)` ↓ | GeoGain / Drift ↑ |
|---|---|---|---|---|---|---:|---:|---:|---:|---:|
| Hard projection + 10% quantile | N/A | N/A | N/A | 否 | 是 | TBD | TBD | TBD | TBD | TBD |
| Hard assignment | 是 | 是 | 是 | 否 | 是 | TBD | TBD | TBD | TBD | TBD |
| Entropic OT, `gamma=0` | 是 | 否 | 否 | 是 | 是 | TBD | TBD | TBD | TBD | TBD |
| Structure-only | 否 | 是 | 是 | 是 | 是 | TBD | TBD | TBD | TBD | TBD |
| Full NTL-FGT w/o `L_sem` | 是 | 是 | 是 | 是 | 否 | TBD | TBD | TBD | TBD | TBD |
| Full NTL-FGT | 是 | 是 | 是 | 是 | 是 | TBD | TBD | TBD | TBD | TBD |

### 4.6 消融实验

#### Native Token–LiDAR Fused Geometric Transport

- hard projection label vs concentration-aware transport teacher；
- hard assignment、Entropic OT（`gamma=0`）与 Full NTL-FGT；
- feature-only、structure-only 与 feature-structure transport；
- coupling 仅使用 `C^(V,fix)` vs 仅使用 `C^(V,pred)` vs fixed-dominant fused structure；
- reliability 外部门控 `q_p^conc g_p`、`q_p^mass g_p` 与 `sqrt(q_p^conc q_p^mass) g_p`；
- high-concentration barycenter vs low-concentration medoid、top-k barycenter 与 front-biased representative；
- with / without `L_sem`；
- global CKA loss vs risk-aware local relation preservation；
- concentration-aware teacher 对 depth、ray 和 coordinate 同时监督 vs 仅监督 depth。

该组实验同时报告 CKA、GeoGain / SemanticDrift、transported mass、coupling entropy，以及 reliability 与 depth error / cross-view accuracy 的相关性；边界、小目标和遮挡区域单独分桶，检验 conservative concentration gate 是否过度降权。优化日程、StopGrad、anchor 类型和 OT 超参数等实现消融移至附录 A.3。
...
- min depth；
- 10% quantile depth；
- FGT concentration-aware representative depth。
...
保持图像与 LiDAR anchor 不变，在 batch 内交换相机内外参并重新计算 feature cost。若 FGT 确实使用标定几何，其 feature transport cost、teacher depth error 和 coupling entropy 应明显上升，`q_p^OT`、3D 坐标和跨视角性能应下降。若 coupling 与最终性能基本不变，则模型可能依赖 camera ID 或数据集位置先验。
...
保持 transport teacher depth 不变，分别在 batch 内打乱 `r_p`、`q_p^geom`、`q_p^OT`、`q_p^teacher` 和组合后的 `a_p^*`，评估近场深度、`30m+` 深度、碰撞率和空间问答。若性能不下降，说明 factorized allocation head 未真正利用对应监督。

#### Transport-structure shuffle

保持 feature cost 与 LiDAR 坐标不变，分别打乱 `C_ij^(L,fix)` 中的 co-visibility、`C_ij^(L,type)` 中的 object/surface、topology 和 occlusion relation，或随机置换 `C_pq^(V,fix)` / `C_pq^(V,pred)`。该实验用于区分固定 coupling structure 与 learned typed supervision 的贡献，不应等价于简单删除全部 metric depth。
...
3. hard quantile depth、concentration-aware transport teacher depth 和 LPGA depth；
...
- **图 3：** Native token measure、LiDAR geometric measure、structured coupling 与 concentration-aware teacher。
...
| LiDAR teacher | 可选目标域校准 | NTL-FGT soft coupling 与 concentration-aware representative |
...
NTL-FGT 同时保留两类约束：feature term 确保 coupling 遵守标定与局部射线几何，fixed structure term 提供不依赖训练状态的成对锚点，learned typed predictor 则在固定 coupling 下学习对象、表面、可见性和遮挡关系。停止梯度日程切断 predictor、coupling 和 metric heads 的循环依赖，细节见附录 A.2。高 concentration token 使用 barycenter，低 concentration token 使用 support medoid；主方法再采用保守 reliability gate，但其是否会过度抑制边界、小目标和遮挡区域由分桶消融验证。该目标是面向 teacher construction 的 FGW-inspired objective，而非严格标准 FGW distance。
...
代价是 spatial merge 降低了深度分辨率。我们通过局部 partial transport、fixed-structure alignment、concentration-aware representative ray 与 sub-token offset 减少前景/背景混合，并将 hard foreground quantile 和 merger 前注入作为消融。

### 5.7 为什么推理时不使用 LiDAR、OT Solver 或 UniDepth

LiDAR、OT solver、coupling construction 与 training-only cost assembly 只在训练阶段提供结构化 teacher。共享 relation edge encoder 与 typed relation heads 保留在推理期，与 LPGA 一起从 Qwen 视觉 token、标定和标准 ego state 预测 metric geometry 与关系。这样部署不需要 LiDAR、coupling 求解或外部深度网络，也不增加环境感知传感器。
...
NTL-FGT 对标定误差、candidate mask 和 LiDAR anchor 构造敏感。熵正则过强可能产生跨 mode 的平滑 coupling，过弱则接近不稳定的 hard assignment；medoid 避免输出跨表面均值，但仍把多层几何压缩为单个 representative。局部 partial coupling、稀疏 structure sampling 与有限次 teacher 更新降低了计算量，但仍增加训练时显存和优化复杂度。本文不主张该 coupling 是唯一真实 correspondence，必须报告 teacher construction overhead、entropy、transported mass、calibration shuffle 与失败案例。
...
Token-level depth 受到 Qwen spatial merger 分辨率限制，同一 merged token 内可能包含前景和背景。High-concentration barycenter、low-concentration medoid 与 sub-token ray offset 缓解了“深度来自前景、射线却固定在区域中心”的结构误差，但单个 token 最终仍只能表达一个代表表面，无法完整保留多层深度。
...
本文提出 GeoDistill-VLM，一种将训练期 LiDAR 几何蒸馏到冻结 Qwen2.5-VL 原生视觉 token 的纯视觉部署框架。方法由三个核心模块组成：NTL-FGT 构造 token-level metric teacher，R²AC 学习风险与可靠度条件化的连续深度表示，semantic-preserving geometry injection 将预测位置和关系分别写入 3D PE 与 relation residual。NTL-FGT 服务于 teacher construction，不声称是严格标准 FGW distance。

模型不训练新的 Vision Encoder，不替换 Qwen merger，也不在推理时运行 LiDAR、OT solver 或外部 UniDepth。轻量 LPGA 从原生 `image_embeds` 预测三维位置与关系，zero-init gate、delayed local relation preservation 和全局语义监控共同限制非必要语义漂移；VLM 阶段仅使用 rank-16 LoRA 微调 Qwen LLM。
...
### A.1 Typed validity mask

`m_(p,q,i,j,k)` 只启用当前边类型下可比较的分量。同相机边启用 image-grid distance、ego-ray angle 和低权重 semantic compatibility cue；跨相机边禁用 image-grid distance，主要使用 ego-ray angle、camera separation/co-visibility 和 semantic compatibility cue。缺少 surface、voxel、object 或 visibility 标签时，对应 LiDAR typed component 置为无效，不以默认负标签代替。frozen-token feature distance 的权重不高于任一几何分量，且不得单独决定 coupling。

### A.2 F0/F1/F2 teacher optimization

```text
Stage F0:
  lambda_d = 0, kappa_pred = 0
  use M_(p,i), C^(V,fix), C^(L,fix)
  initialize pi^(0) with sparse Sinkhorn/proximal OT

Stage F1:
  freeze pi = StopGrad(pi^(0))
  train shared A_edge and typed relation heads A_struct^V
  with L_struct^pred and L_rel

Stage F2:
  update pi with C^(V,fix) and
  kappa_pred StopGrad(C^(V,pred))
  freeze pi = StopGrad(pi)
  construct token teacher and supervise LPGA
```

F0 使用固定 feature/ray/camera geometry 与非学习 token structure 初始化 coupling。F1 固定 coupling，仅训练共享 typed relation predictor。F2 更新 coupling 时停止 `C^(V,pred)` 的梯度，并在 teacher construction 后再次停止 `pi` 的梯度。实现只在局部 LiDAR candidates 和稀疏 token edges 上估计结构项，并执行有限次 F1/F2 更新。LiDAR、cost assembly、coupling 与 OT solver 均不进入推理路径。

### A.3 NTL-FGT 实现消融

- F0 only、F0+F1 与完整 F0/F1/F2；
- 移除 `StopGrad(pi)` 或 `StopGrad(C^(V,pred))`；
- balanced OT vs partial/relaxed marginal；
- point、voxel 与 local surface anchor；
- 不同 `epsilon_ot`、`gamma`、`m_tr`、candidate anchor 数和 Sinkhorn 迭代数；
- `q_p^OT=q_p^conc` vs `q_p^OT=q_p^conc q_p^mass`；
- 不同 `tau_conc` 和 medoid top-k；
- `lambda_sem` 从训练开始启用 vs geometry warm-up 后 ramp；
- 主方法 `lambda_d=0` vs late-stage stopped-gradient depth prior。
