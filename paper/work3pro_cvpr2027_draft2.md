...
Camera-only driving vision-language models (VLMs) have shown promising semantic understanding and language-action alignment, but they remain unreliable in metric spatial reasoning, cross-view geometry, and adverse sensing conditions. This paper studies whether training-time LiDAR can be used as a privileged spatial teacher to produce camera-only visual tokens that are more useful for driving VLMs. We propose **GeoToken**, a LiDAR-privileged pretraining framework for multi-view camera video representation learning. Instead of directly regressing LiDAR features, GeoToken introduces **Privileged Spatial Relation Distillation (PSRD)**, which converts LiDAR observations into metric, directional, and occupancy-topology relations, and distills these relations into camera token interactions. To make these geometric tokens more accessible to language models, we further introduce **Language-Grounded Spatial Slots (LGSS)**, a lightweight weak-language alignment objective that binds spatial tokens to automatically generated semantic-spatial phrases such as "a nearby vehicle in front." At inference time, GeoToken uses only multi-view camera frames and outputs spatial tokens that can be connected to a frozen VLM through a lightweight adapter. We evaluate GeoToken on driving spatial question answering, spatial referring, frozen spatial probing, and robustness under sensor degradation. The intended empirical claim is that LiDAR-privileged spatial relation pretraining, augmented with weak semantic-spatial grounding, improves camera-only spatial reasoning, cross-view grounding, and degradation robustness. If validated, this would suggest that training-time 3D sensing and low-cost weak language supervision provide a practical path toward spatially grounded camera-only driving VLMs without requiring LiDAR at deployment.
...
We propose **GeoToken**, a LiDAR-privileged spatial token pretraining framework for camera-only driving VLMs. GeoToken trains a multi-view camera video tokenizer whose output spatial tokens can be used for VLM spatial perception and spatial reasoning tasks. The core of GeoToken is **Privileged Spatial Relation Distillation (PSRD)**. PSRD first converts LiDAR observations and ego pose into a spatial relation graph over visible camera tokens. The graph encodes metric distance, relative direction, visibility, occlusion order, and occupancy-topology relations. The camera tokenizer is then trained to reproduce these privileged relations through token-token interactions. Thus, the student does not simply copy LiDAR features; it learns how visual tokens should be organized according to 3D driving geometry.

However, geometry alone is not always easy for a language model to index. A VLM often needs a joint representation of semantic anchors and spatial references, such as "the vehicle 15 meters ahead" or "the pedestrian on the right is closer than the front vehicle." To address this interface gap, GeoToken further introduces **Language-Grounded Spatial Slots (LGSS)**. LGSS uses automatically generated weak phrases from structured driving metadata, 3D boxes, and LiDAR-derived distance and direction bins. These phrases provide low-cost semantic-spatial anchors without requiring manual VQA annotation. During inference, the text encoder is removed and the camera-only slot tokens are passed to the VLM adapter.
...
3. We introduce **Language-Grounded Spatial Slots**, which align spatial tokens with automatically generated semantic-spatial phrases, making the learned geometry more directly indexable by frozen VLMs.
4. We propose a VLM-centered evaluation protocol covering driving spatial QA, spatial referring, frozen depth/BEV/relation probing, and robustness under camera degradation and adverse conditions.
...
### Weak Language Grounding for Driving

Driving datasets contain structured metadata that can be converted into low-cost language supervision, including object categories, 3D boxes, ego-relative location, distance bins, and scene tags. Such weak language does not replace human-annotated VQA, but it can align visual features with semantic-spatial phrases that are easy for VLMs to index. GeoToken uses this idea conservatively: weak phrases are only used during pretraining to bind spatial slots to object-centric and region-centric descriptions, while LiDAR-derived relations remain the main geometric teacher.
...
PSRD teaches camera tokens how to organize themselves according to 3D geometry. However, a VLM consumes visual information through language-conditioned attention and benefits from discrete semantic-spatial anchors. We therefore add a lightweight weak-language alignment module, **Language-Grounded Spatial Slots (LGSS)**.

Given spatial tokens \(Z\), we use \(K\) learnable slot queries to read out semantic-spatial slots:

$$
S = \mathrm{CrossAttn}(Q_{slot}, Z) \in \mathbb{R}^{K \times C}.
$$

Each slot is encouraged to correspond to an object, region, or relation that can be described by a short spatial phrase. We construct weak phrases from structured driving metadata and LiDAR-derived geometry. For an object with category \(c\), ego-relative direction \(b\), and distance bin \(r\), we generate templates such as:

$$
p = \mathrm{Template}(b, r, c),
$$

for example "a nearby vehicle in front" or "a pedestrian on the right." Region phrases such as "drivable space on the left" can be generated from map or occupancy labels when available. We encode phrases using a frozen text encoder \(E_{txt}\):

$$
u = E_{txt}(p).
$$

The slot-phrase contrastive loss aligns matching slots and phrases within a batch:

$$
\mathcal{L}_{slot}
=
- \log
\frac{\exp(\mathrm{sim}(s_i, u_i)/\tau)}
{\sum_j \exp(\mathrm{sim}(s_i, u_j)/\tau)}.
$$

We also generate weak spatial relation QA from 3D boxes and LiDAR geometry. For example, if object \(i\) is closer than object \(j\), the pseudo question "which object is closer?" receives the answer \(i\). Instead of training the full language model, we supervise a small relation head over slot pairs:

$$
\mathcal{L}_{qa}
=
\mathrm{CE}(\psi([s_i, s_j]), y_{ij}).
$$

Finally, we add geometry-language consistency to prevent phrase alignment from drifting into pure semantics. If a phrase contains a direction or distance bin, the corresponding slot must predict the same bin under the LiDAR-derived labels:

$$
\mathcal{L}_{geo\text{-}txt}
=
\mathrm{CE}(\hat{b}_i, b_i)
+ \mathrm{CE}(\hat{r}_i, r_i).
$$
...
+ \lambda_{qa}\mathcal{L}_{qa}
+ \lambda_{gt}\mathcal{L}_{geo\text{-}txt}.
$$

The text encoder and weak phrases are used only during training. At inference time, GeoToken outputs camera-only spatial tokens and slot tokens. The slots act as semantic-spatial anchors for downstream VLM adapters.
...
To evaluate whether GeoToken is useful for VLMs, we connect pretrained spatial tokens and slot tokens to a frozen driving VLM through a lightweight adapter:
...
**Weak semantic-spatial phrases.** For LGSS pretraining, we automatically generate phrase labels from nuScenes 3D boxes, object categories, ego-relative positions, distance bins, and scene tags. These labels are used only during pretraining and are not required at inference.
...
We evaluate whether the learned slots can be indexed by language. Given a semantic-spatial phrase such as "a nearby vehicle in front," the model must select the correct slot, projected patch, or object among candidates. We report referring accuracy and phrase-to-slot retrieval Recall@K. To avoid circular evaluation, we include both template-style phrases and paraphrased phrases whose surface forms differ from the training templates.
...
9. GeoToken with PSRD + LGSS.
10. GeoToken with PSRD + shuffled weak phrases, used as a negative control for language grounding.
...
| GeoToken w/o LGSS | TBD | TBD | TBD | TBD | TBD | TBD | TBD |
| GeoToken + shuffled phrases | TBD | TBD | TBD | TBD | TBD | TBD | TBD |
| GeoToken + LGSS | TBD | TBD | TBD | TBD | TBD | TBD | TBD |

#### Spatial Referring

Table 2 evaluates whether spatial slots are language-indexable.

**Table 2. Spatial referring and phrase-to-slot retrieval.**

| Method | Referring Acc | Phrase R@1 | Phrase R@5 | Paraphrase Acc |
|---|---:|---:|---:|---:|
| DINOv2 adapter | TBD | TBD | TBD | TBD |
| GeoToken w/o LGSS | TBD | TBD | TBD | TBD |
| GeoToken + shuffled phrases | TBD | TBD | TBD | TBD |
| GeoToken + LGSS | TBD | TBD | TBD | TBD |

#### Frozen Spatial Probing

Table 3 evaluates whether spatial information is present in frozen tokens without VLM fine-tuning.

**Table 3. Frozen spatial probing.**
...
Table 4 reports average performance under camera and weather degradation.

**Table 4. Robustness under sensor degradation.**
...
| GeoToken w/o LGSS | TBD | TBD | TBD | TBD | TBD | TBD |
| GeoToken + LGSS | TBD | TBD | TBD | TBD | TBD | TBD |

### 4.5 Ablation Study

We isolate the contribution of each PSRD component.

**Table 5. PSRD and LGSS ablation.**

| Variant | Spatial QA | Referring Acc | Relation Probe | BEV IoU | Robust QA |
|---|---:|---:|---:|---:|---:|
| No LiDAR privileged supervision | TBD | TBD | TBD | TBD | TBD |
| Depth-only distillation | TBD | TBD | TBD | TBD | TBD |
| BEV-only distillation | TBD | TBD | TBD | TBD | TBD |
| Descriptor-only distillation | TBD | TBD | TBD | TBD | TBD |
| PSRD without metric distance | TBD | TBD | TBD | TBD | TBD |
| PSRD without direction | TBD | TBD | TBD | TBD | TBD |
| PSRD without topology | TBD | TBD | TBD | TBD | TBD |
| Full PSRD w/o LGSS | TBD | TBD | TBD | TBD | TBD |
| Full PSRD + shuffled phrases | TBD | TBD | TBD | TBD | TBD |
| Full PSRD + slot-phrase only | TBD | TBD | TBD | TBD | TBD |
| Full PSRD + LGSS | TBD | TBD | TBD | TBD | TBD |

We also evaluate clip length, number of cameras, and adapter capacity:

**Table 6. Architecture and input ablation.**
...
- **Figure 4:** LGSS alignment between semantic-spatial phrases and slot tokens.
- **Figure 5:** VLM adapter evaluation protocol with frozen VLM.
- **Figure 6:** Qualitative token affinity and slot retrieval under clean and degraded inputs.
...
### Why Weak Language Grounding?

PSRD gives tokens metric and topological structure, but a VLM still needs language-indexable anchors. LGSS provides those anchors using low-cost templates from existing driving metadata. The shuffled-phrase ablation is important: if language grounding is meaningful, shuffled phrases should not provide the same gain as geometrically consistent semantic-spatial phrases.

### Deployment Cost

GeoToken requires LiDAR and weak phrases only during training. Inference uses only camera video and a lightweight token adapter, making it compatible with camera-only deployment.

---

## 6. Limitations

GeoToken depends on paired camera-LiDAR data during pretraining. Datasets without LiDAR cannot directly provide PSRD supervision. The relation graph also depends on calibration quality and LiDAR visibility; sparse or noisy LiDAR may provide incomplete supervision for distant or heavily occluded regions. LGSS uses template-based weak phrases, which may introduce linguistic bias and cannot replace diverse human-written driving questions. Finally, while frozen VLM spatial QA is a strong proxy for VLM usefulness, it does not replace full closed-loop driving evaluation. Future work should integrate GeoToken with closed-loop VLA systems and evaluate planning behavior directly.

---

## 7. Conclusion

We presented GeoToken, a LiDAR-privileged spatial token pretraining framework for camera-only driving VLMs. The core method, Privileged Spatial Relation Distillation, transfers metric, directional, and occupancy-topology relations from LiDAR to camera token interactions. We further introduced Language-Grounded Spatial Slots to bind geometric tokens with weak semantic-spatial phrases. GeoToken is designed to improve spatial reasoning, language-indexed spatial grounding, and robustness while requiring only cameras at inference. Through VLM spatial QA, spatial referring, frozen spatial probing, and robustness evaluation, this work aims to establish training-time 3D sensing and weak language grounding as a practical route toward spatially grounded camera-only driving VLMs.
